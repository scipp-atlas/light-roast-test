...
| `make_abcd_json.py` | Processes ROOT picontuples into per-sample ABCD JSON result files. Walks the ntuple directory and writes one JSON per (sample, ID, Iso, LoosePrime) combination. | JSON result files | `ABCD_results_<tag>/` |
| `print_abcd_estimates.py` | Prints ABCD background estimates (data-driven JFP, MC JFP, and signal yields) for SR, VR, and Preselection regions from pre-computed JSON files. Optionally runs per-sample closure tests and saves pull plots. Reads output of `make_abcd_json.py`. | Background estimate tables printed to stdout; per-sample pull plots (PDF) | stdout; `ABCD_results_<tag>/` |
...
| `rprime_analysis.py` | Computes R′ from picontuples as a function of photon pT, \|η\|, conversion status, process, and region. Supports pT-dependence corrections, systematic uncertainty tables, and LaTeX output. | PDF/PNG plots; optional `.tex` table files | `rprime_analysis_output/` |
